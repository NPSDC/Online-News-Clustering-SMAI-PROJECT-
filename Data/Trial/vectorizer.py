...
class CountVectorizer(BaseEstimator, VectorizerMixin):
    ...
    def transform(self, raw_documents):
        ...
        _, X = self._count_vocab(raw_documents, fixed_vocab=self.fixed)
        if self.binary:
            X.data.fill(1)
        return X
...
def main():
    T1 = TfidfTransformer()
    T2 = TfidfTransformer()
    vectorizer_ = CountVectorizer(min_df=1, fixed=True)
    ...
    vectorizer_ = CountVectorizer(min_df=1, fixed=True)
    count = vectorizer_.fit_transform(corpus)
    t2 = T2.fit_transform(count)
    print t2.shape

    V = vectorizer_.vocabulary_

    vectorizer_ = CountVectorizer(min_df=1, vocabulary=V, fixed=True)
    count = vectorizer_.fit_transform(corpus[:20])
    t1 = T1.fit(count)
    count = vectorizer_.transform(corpus[20:])
    t1 = T1.fit(count)
    t1 = T1.transform(count)
    print t1.shape

    print (t1.toarray() - t2.toarray()[20:]).sum(axis=0).sum(axis=0)
