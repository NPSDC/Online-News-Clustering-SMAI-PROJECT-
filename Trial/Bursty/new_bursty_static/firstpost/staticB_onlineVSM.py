...
class BVsmVectorizer(object):
	...
	def getBurstyWords(self, count, vocab):
		...
		for w, i in vocab.items():
			...
			if net_cost - math.log(self.wordWise[i]) > 0. and improvmt > 0.:
				...
				if net_cost > self.bursts[w]:
					self.bursts[w] = net_cost
			else:
				if net_cost > 0.:
					self.bvocab[i][0] = net_cost
				else:
	...
	def count(self, corpus, vocab=None):
		count = self.vectorizer.fit_transform(corpus)
		self.vocab = self.vectorizer.vocabulary_
		N  = count.shape[0]         	# number of documents on this day
		W  = count.shape[1]         	# number of words on this day
		...
		self.wordWise  = np.lib.pad(self.wordWise, pad_width=(0, newwords), \
			                        mode='constant', constant_values=0 )
		self.wordWise = self.wordWise + df.transpose()
		self.N += N
		self.W += newwords
...
def getAverages():
	...
	for d in allDates:
		...
		try:
			...
			BV.count(dcorpus, vocab=vocabulary)
			vocabulary = BV.vocab
		except KeyboardInterrupt:
			print "Could Not Vectorise", d
			exit()
		...
		print d, "\n"
		if not i % 10:
			pickle.dump( (BV.vocab, BV.wordWise, d), open("vectors/vectorsrizer."+str((d/10)%2), "wb") )
	
	print i , (i/10)%2
	pickle.dump( (BV.vocab, BV.wordWise, d), open("vectors/vectorizer."+str(d), "wb") )
