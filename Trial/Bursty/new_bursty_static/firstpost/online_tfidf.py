...
class BVsmVectorizer(object):
	...
	def __init__(self, vocab, bwords=None):
		self.vocab  = dict()
		self.bvocab = dict()
	...
	def getBurstyWords(self, count, vocab):
		...
		self.W += newwords
		self.bwords += bwords
		self.bwords = list(set(self.bwords))
		print len(self.bwords)
	...
	def vectorize(self, corpus, vocab=None):
		c   = self.vectorizer.fit_transform(corpus)
		self.vocab = self.vectorizer.vocabulary_
		self.getBurstyWords(np.float64(c.toarray(), dtype=np.float64), self.vocab)
		t   = self.tfidf(c)
		b   = self.bness()
		sab = self.sab(t, b)
		smb = self.smb(t, b)
		return c, t, b, sab, smb
